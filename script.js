...
(function () {
  ...
  if (range) {
    ...
    var TIERS = [
      { upTo: 10,  key: "starter",   name: "Starter",   intro: 130, standard: 185 },
      { upTo: 30,  key: "growth",    name: "Growth",    intro: 110, standard: 155 },
      { upTo: 60,  key: "scale",     name: "Scale",     intro: 90,  standard: 125 },
      { upTo: 100, key: "portfolio", name: "Portfolio", intro: 75,  standard: 105 }
    ];
    ...
    function updatePricing() {
      ...
      Object.keys(tiers).forEach(function (k) {
        if (tiers[k]) tiers[k].classList.toggle("active", !isEnterprise && k === t.key);
      });

      if (isEnterprise) {
        readout.innerHTML = '<span class="psc-total">custom enterprise pricing</span>';
        if (effLine) { effLine.innerHTML = '<span id="pscUnits">' + u + "</span> units"; }
        if (pscStandard) { pscStandard.hidden = true; }
        if (pscIntroLabel) { pscIntroLabel.hidden = true; }
        if (pscBreakdown) { pscBreakdown.hidden = true; }
